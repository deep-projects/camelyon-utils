...
def main():
    ...
    if MODE == 'single':
        ...
        merged_tumor_dset_val = merged_h5.create_dataset(name='tumor_valid', shape=(len(f_t_valid)*NUM, 512, 512, 3), dtype=np.uint8)
        merged_normal_dset_val = merged_h5.create_dataset(name='normal_valid', shape=(len(f_n_valid)*NUM, 512, 512, 3), dtype=np.uint8)
        ...
        for i in range(NUM):
            ...
            for j in range(len(h5_t_train)):
                if h5_t_train[j].shape[1] >= 1:
                    ...
                    new_t_train[j] = new_t_train[np.random.randint(0,len(new_t_train))]
            ...
            for j in range(len(h5_n_train)):
                if h5_n_train[j].shape[1] >= 1:
                    ...
                    new_n_train[j] = new_n_train[np.random.randint(0,len(new_n_train))]
            ...
            for j in range(len(h5_t_val)):
                if h5_t_val[j].shape[1] >= 1:
                    ...
                    new_t_val[j] = new_t_val[np.random.randint(0,len(new_t_val))]
            ...
            for j in range(len(h5_n_val)):
                if h5_n_val[j].shape[1] >= 1:
                    ...
                    new_n_val[j] = new_n_val[np.random.randint(0,len(new_n_val))]
