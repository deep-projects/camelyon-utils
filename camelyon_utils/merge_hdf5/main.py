        ...
        for i in range(NUM):
            ...
            for j in range(len(h5_t_train)):
                if h5_t_train[j].shape[1] >= 1:
                    new_t_train[j,:,:,0:-2] = h5_t_train[j][0,np.random.randint(0,h5_t_train[j].shape[1])]
                    new_t_train[j,:,:,-1] = h5_t_train_mask[j][0,np.random.randint(0,h5_t_train[j].shape[1])]
                else:
                    new_t_train[j,:,:,0:-2] = new_t_train[np.random.randint(0,len(new_t_train)),:,0:-2]
            ...
            for j in range(len(h5_n_train)):
                if h5_n_train[j].shape[1] >= 1:
                    new_n_train[j,:,:,0:-2] = h5_n_train[j][0,np.random.randint(0,h5_n_train[j].shape[1])]
                    new_n_train[j,:,:,-1] = h5_n_train_mask[j][0,np.random.randint(0,h5_n_train[j].shape[1])]
                else:
                    new_n_train[j,:,:,0:-2] = new_n_train[np.random.randint(0,len(new_n_train)),:,0:-2]
            ...
            for j in range(len(h5_t_val)):
                if h5_t_val[j].shape[1] >= 1:
                    new_t_val[j,:,:,0:-2] = h5_t_val[j][0,np.random.randint(0,h5_t_val[j].shape[1])]
                    new_t_val[j,:,:,-1] = h5_t_val_mask[j][0,np.random.randint(0,h5_t_val[j].shape[1])]
                else:
                    new_t_val[j,:,:,0:-2] = new_t_val[np.random.randint(0,len(new_t_val)),:,0:-2]
            ...
            for j in range(len(h5_n_val)):
                if h5_n_val[j].shape[1] >= 1:
                    new_n_val[j,:,:,0:-2] = h5_n_val[j][0,np.random.randint(0,h5_n_val[j].shape[1])]
                    new_n_val[j,:,:,-1] = h5_n_val_mask[j][0,np.random.randint(0,h5_n_val[j].shape[1])]
                else:
                    new_n_val[j,:,:,0:-2] = new_n_val[np.random.randint(0,len(new_n_val)),:,0:-2]
